...
#        print(conf)
            ...
            #psf_OFF=np.repeat(psf_OFF[:,np.newaxis,:,:],2000,axis=1)
        ...
        #fits.writeto("psf_on_scopesim_{0}_{1}.fits".format(hdu.header["MODE"],hdu.header["FILTER"]),psf_ON,overwrite=1)
        #fits.writeto("psf_off_scopesim_{0}_{1}.fits".format(hdu.header["MODE"],hdu.header["FILTER"]),psf_OFF,overwrite=1)
