...
# parallactic angle in deg
    ...
    if conf['ScopeSim_LMS']==False:
        ...
        newheader_ON.writeto("psf_on_scopesim_{0}_{1}.fits".format(header["MODE"],header["FILTER"]),overwrite=True)
        newheader_OFF.writeto("psf_off_scopesim_{0}_{1}.fits".format(header["MODE"],header["FILTER"]),overwrite=True)
        newheader_SUM.writeto("psf_sum_scopesim_{0}_{1}.fits".format(header["MODE"],header["FILTER"]),overwrite=True)
